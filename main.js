fetch("./data.json")
  ...
  .then((galleryList) => {
    let storage = JSON.parse(localStorage.getItem("myFav"));
    if(storage) {
      galleryList = storage;
    }
    const render = (str, mm, favInfo) => {
      $(str).html("");
      console.log(favInfo);
      mm.forEach((item, i) => {
        $(str).append(`<div class="img-box ${favInfo}" id="img-box${i}">
        <img src="${item.img[0]}" alt="gallery image"/>
          <button onclick='fav(${i})' id='rem'><i class='far fa-trash-alt'></i></button>
          <div class="text">
            <p><b>${item.title}</b></p>
          </div>
      </div>`);
        $("#img-box" + i).click(() => {
          ...
          $(".page2").append(`<div class="container2">
              <div class="descTitle">${mm[i].title}<span class='fav' onclick='fav(${i})'><img src='${src}'/></span></div>
              <div class="desc">
                ${mm[i].desc}
              </div>
    ...
    let myFav, favRemoveInfo;

    render(".gallery-image", galleryList, favRemoveInfo);

    const renderFavourite = () => {
      $(".favGallery").html("");
      myFav = galleryList.filter((item) => item.isFavourite);
      if (myFav.length) {
        favRemoveInfo = "favBox";
        render(".favGallery", myFav, favRemoveInfo);
      } else {
        $(".favGallery").append(
          `<div class='no_fav'><h3>No favourites yet!</h3></div>`
        );
      }
    };
    ...
    fav = (i) => {
      ...
      if ($(".favGallery")[0]) {
        myFav[i].isFavourite = !myFav[i].isFavourite;
      }
    ...
    const search = () => {
      $(".searchTerm").on("keyup", function () {
        ...
        render(".gallery-image", filteredData, favRemoveInfo);
      });
    };
